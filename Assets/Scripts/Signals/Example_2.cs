...
//	private Dropdown InputDropdown;
	private BeaconType bt_PendingType = BeaconType.iBeacon;
...
//	private float f_ScrollViewContentRectHeight;
	...
	public void Digl_Enable_Bluetooth(){
...
//		InputDropdown.value = (int)bt_Type;
	...
	public void Digl_Change_Data(string region, string uuid, string major, string minor){
		s_Region = region;
...
//		}
	...
	public void Digl_SetMode(BroadcastMode newMode){
		bm_Mode = newMode;
		Debug.Log ("DIGL setMode "+newMode);
		SetBroadcastMode();
		SavePlayerPrefs();
	}
...
//			txt_BroadcastMode_ButtonText.text = "Switch to " + BroadcastMode.receive.ToString();
	...
	// BroadcastState
	public void btn_StartStop(bool digl_activate) {
		Debug.Log ("DIGL btn_StartStop");
...
//					input_Region.image.color= Color.red;
					Diglbug.Log("BLE ERROR: Rec Region null", PrintStream.SIGNALS);
					Diglbug.LogMobile ("BLE ERROR: Rec Region null", "BLE_EG_2");
					return;
				}
				if (bt_Type == BeaconType.Any) {
					...
					if (s_UUID == null || s_UUID == "") {
						...
						Diglbug.Log("BLE ERROR: Rec UUID null", PrintStream.SIGNALS);
						Diglbug.LogMobile ("BLE ERROR: Rec UUID null", "BLE_EG_2");
						return;
					}
					if (bt_Type == BeaconType.iBeacon) {
			...
			// SendMode
				...
				if (s_Region == null || s_Region == "") {
//					input_Region.image.color= Color.red;
					Diglbug.Log("BLE ERROR: Send Region null", PrintStream.SIGNALS);
					Diglbug.LogMobile ("BLE ERROR: Send Region null", "BLE_EG_2");
					return;
				}
...
//					InputDropdown.image.color = Color.red;
				...
				if (bt_Type == BeaconType.Any) {
					...
					if (s_UUID == null || s_UUID == "") {
//						input_UUID.image.color= Color.red;
						Diglbug.Log("BLE ERROR: Send UUID null", PrintStream.SIGNALS);
						Diglbug.LogMobile ("BLE ERROR: Send UUID null", "BLE_EG_2");
						return;
					}
					if (bt_Type == BeaconType.EddystoneURL) {
...
//							input_Major.image.color= Color.red;
							Diglbug.Log("BLE ERROR: Send Major null", PrintStream.SIGNALS);
							Diglbug.LogMobile ("BLE ERROR: Send Major null", "BLE_EG_2");
							return;
						}
...
//								input_Major.image.color= Color.red;
								Diglbug.Log("BLE ERROR: Send Minor null", PrintStream.SIGNALS);
								Diglbug.LogMobile ("BLE ERROR: Send Minor null", "BLE_EG_2");
								return;
							}
...
//		txt_BroadcastState_LabelText.text = bs_State.ToString();
	...
	public delegate void DiglBeaconRangeChangeDelegate(Beacon[] beacons);
	public event DiglBeaconRangeChangeDelegate DiglBeaconRangeChangeEvent;
...
// DIGL
		if (DiglBeaconRangeChangeEvent != null) {
			DiglBeaconRangeChangeEvent (beacons);

			Debug.Log ("DIGL beaconFound");
		}
	}
